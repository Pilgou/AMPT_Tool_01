    ...
    if not st.session_state.df.empty:
        ...
        if st.session_state.checkbox_Curves_dftab_2:
            with st.form(key = "Output_Analyse"):
                ...
                st.session_state.form_submit_button = st.form_submit_button(label = "Trace")
...
colonnesforstats = ['OutDCV_1', 'In1DCV_1', 'In2DCV_1',
               'OutDCV_2', 'In1DCV_2', 'In2DCV_2',
               'OutDCV_3',
             'In1DCV_3', 'In2DCV_3',
              'OutDCV_4', 'In1DCV_4', 'In2DCV_4',
             'OutDCV_5', 'In1DCV_5', 'In2DCV_5',
              'OutDCV_6',
             'In1DCV_6', 'In2DCV_6']

if not st.session_state.df.empty:
    if st.session_state.checkbox_Anomalies_dftab_2:
        st.markdown("Statistics on Voltages")
        for col in colonnesforstats:
            ...
            mean_val = desc["min"]
            seuil = 1e-6 
            if np.isclose(mean_val, 0, atol=seuil):  # tolérance 1e-6
                stat_error = "⚠️"
